...
class Books extends CI_Controller {
	...
	public function index()
	{
		...
		$total_records = $this->books_model->get_books_total($keyword);
		$data['total_books'] = $total_records;
		$data['start_index'] = $start_index;

		if($total_records > 0) {
			$data['books'] = $this->books_model->get_books($limit_per_page, $start_index, $keyword);
	...
	public function search()
	{
		...
		if($total_records > 0) {
			...
			$config['base_url'] = admin_url() . 'books/search';
			$config['total_rows'] = $total_records;
			$config['per_page'] = $limit_per_page;
			$config["uri_segment"] = 4;
	...
	public function export_used_books(){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('B1', 'University');
		$objPHPExcel->getActiveSheet()->setCellValue('C1', 'Field Of Study');
		$objPHPExcel->getActiveSheet()->setCellValue('D1', 'Full Time Semester');
		$objPHPExcel->getActiveSheet()->setCellValue('E1', 'Part Time Semester');
		$objPHPExcel->getActiveSheet()->setCellValue('F1', 'Mandatory/Optional');
		$objPHPExcel->getActiveSheet()->setCellValue('G1', 'Price');
		$objPHPExcel->getActiveSheet()->setCellValue('H1', 'ISBN');
		$objPHPExcel->getActiveSheet()->setCellValue('I1', 'Book Name');
		$objPHPExcel->getActiveSheet()->setCellValue('J1', 'Extra Information');
		$objPHPExcel->getActiveSheet()->setCellValue('K1', 'Module');


		$used_books = $this->books_model->export_used_books();
		$excel_row = 2;
		foreach($used_books as $book){
			if (!empty($book->first_name)) {
				$full_name = $book->first_name.' '.$book->last_name;
			}else{
				$full_name = $book->email;
			}

			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $excel_row, $full_name);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $excel_row, $book->university);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $excel_row, $book->field_of_study);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $excel_row, $book->full_time_semester);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $excel_row, $book->part_time_semester);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $excel_row, $book->mandatory_or_optional);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, $book->price.' '.'CHF');
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, $book->ISBN);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $book->book_name);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, $book->extra_information);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $book->module);
			$excel_row++;
		}
		$objPHPExcel_writer = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
		$filename = 'used-books-list'.time().'.xlsx';
