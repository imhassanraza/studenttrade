    ...
    <div class="page-wrapper">
        <div class="page-header">
            ...
            <div class="page-header-breadcrumb">
                <a href="<?php echo admin_url(); ?>orders/get_cancelled_orders" class="btn btn-inverse btn_inverse_white"><i class="fa fa-download"></i> Export Orders</a>
            </div>
        </div>
                                    ...
                                    <?php $i= 1; foreach ($orders as $order) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <!-- <button class="btn btn-danger btn-sm refund_order_payment" data-id="<?php // echo $order['id']; ?>" type="button" data-toggle="tooltip" data-placement="top" title="Refund"><i class="fa fa-share"></i> Refund</button> -->
                                            </td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
        ...
        <div class="modal" id="refund_modalbox">
            <div class="modal-dialog modal-md">
                <div class="modal-content" id="refund_modalbox_body"></div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).on("click" , ".refund_order_payment__remove" , function() {
        var id = $(this).attr('data-id');
        $.ajax({
            url:'<?php echo admin_url(); ?>orders/refund_order_payment',
            type: 'POST',
            data: { id : id},
            dataType:'json',
            success:function(status){
                if(status.msg=='success'){
                    $('#refund_modalbox_body').html(status.response);
                    $('#refund_modalbox').modal('show');
                }
                else if(status.msg == 'error'){
                    notify('Error! ', status.response, 'danger');
                }
    ...
    $(document).on("click" , ".SubmitRefundPaymentCustomer" , function() {
        ...
        $.ajax({
            url:'<?php echo admin_url(); ?>orders/add_refund_payment',
            type: 'POST',
