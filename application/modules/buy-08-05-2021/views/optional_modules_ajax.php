<table class="table">
	<thead>
		<tr>
			...
			<th width="25%"><?php echo lang('module'); ?>
			<?php if (get_session('university') == 'ZHAW') { ?>
				<span class="tooltiper" data-tooltip="<?php echo lang('required_books_display'); ?>"><i class="fa fa-info-circle" aria-hidden="true"></i></span>
			<?php } ?>
		</th>
		<th width="40%"><?php echo lang('book_name'); ?></th>
		<th width="10%"><?php echo lang('condition'); ?></th>
		<!-- <th width="5%"><?php //echo lang('mandatory'); ?></th> -->
...
<tbody>
	<?php if(empty($books)) { ?>
		<tr>
			<td colspan="6">
				<?php if (get_session('university') == 'ZHAW') { ?>
					<span style="color: red;"><?php echo lang('optional_module_books_not_found'); ?></span>
				<?php } else{ ?>
	...
	<?php foreach ($books as $book) {
		...
		<tr>
			...
			<!-- <td><?php //if(!$book['mandatory_or_optional']) { ?> <?php // echo lang('yes'); ?> <?php // } else { ?>  <?php // echo lang('no'); ?> <?php // } ?></td> -->
			<td>
				<?php if($is_in_cart) { ?>
